...
if (isset($_POST['excluirConta'])) {
    ...
    try {
        $queryPagarpf = "DELETE FROM pagarpf WHERE email = ?";
        $stmtPagarpf = mysqli_prepare($conexao, $queryPagarpf);
        mysqli_stmt_bind_param($stmtPagarpf, 's', $email);
        mysqli_stmt_execute($stmtPagarpf);

        $queryPessoa = "DELETE FROM pessoa WHERE email = ?";
        $stmtPessoa = mysqli_prepare($conexao, $queryPessoa);
        mysqli_stmt_bind_param($stmtPessoa, 's', $email);
        mysqli_stmt_execute($stmtPessoa);
...
<body>
    <div class="container">
        ...
        <p><a href="pagdecripto.php">Voltar</a></p>
    </div>
</body>
</html>
